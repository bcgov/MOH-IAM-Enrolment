...
```console
oc policy add-role-to-user edit system:serviceaccount:61ab99-tools:default \
  -n $(oc project --short)
```
...
```
oc policy add-role-to-user system:image-puller system:serviceaccount:61ab99-dev:default --namespace=61ab99-tools
```
...
8. The following instructions are for the build and deployment of the `iam` component. The build uses the on-cluster `nodejs:10` S2I image to run any scripts from `package.json` which require node. This step produces an artifacts image (iam-web-artifacts) that used as part of chained build. These artifacts are (from `npm run build`) are then consumed by the NGINX image which is pulled in from the RedHat Container Registry. This results in an image named `iam-web` that can be deployed.

The deployment mounts a `ConfigMap` containing the necessary NGINX config.

### Build

9. The GitHub Workflow (Actions) will use `oc` to trigger commands on-cluster. This workflow is located [here](../.github/workflows/iam.yml) in the `.github/workflows` folder of this project.
...
```console
oc process -f sitereg/openshift/templates/build.yaml | \
  oc create -f -
```
