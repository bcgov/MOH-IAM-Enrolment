...
export function jsonPayLoadApplication(stateService: UpdateStateService) {
    ...
    if (stateService.forms.requestorForm) {
        ...
        requestor = appRequestor.getJSONofRequestor(
            stateService.forms.requestorForm.value,
    ...
    if (stateService.forms.organizationForm) {
        ...
        organization = appOrganization.getEditJSONofOrganization(
            stateService.forms.organizationForm.value
        );
    ...
    if (
        stateService.forms.signingAuthority &&
        stateService.forms.signingAuthority.add
    ) {
        singingAuthorityAdd = appSA.getAddJSONofSigningAuthority(
            stateService.forms.signingAuthority.add.value
        );
    ...
    if (
        stateService.forms.signingAuthority &&
        stateService.forms.signingAuthority.update
    ) {
        singingAuthorityEdit = appSA.getEditJSONofSigningAuthority(
            stateService.forms.signingAuthority.update.value
        );
    ...
    if (
        stateService.forms.signingAuthority &&
        stateService.forms.signingAuthority.remove
    ) {
        singingAuthorityRemove = appSA.getRemoveJSONofSigningAuthority(
            stateService.forms.signingAuthority.remove.value
        );
    ...
    if (
        stateService.forms.mspAccessAdministrators &&
        stateService.forms.mspAccessAdministrators.add
    ) {
        accessAdministratorAdd = appAA.getAddJSONofAccessAdministrator(
            stateService.forms.mspAccessAdministrators.add.value
        );
    ...
    if (
        stateService.forms.mspAccessAdministrators &&
        stateService.forms.mspAccessAdministrators.update
    ) {
        accessAdministratorEdit = appAA.getEditJSONofAccessAdministrator(
            stateService.forms.mspAccessAdministrators.update.value
        );
    ...
    if (
        stateService.forms.mspAccessAdministrators &&
        stateService.forms.mspAccessAdministrators.remove
    ) {
        accessAdministratorRemove = appAA.getRemoveJSONofAccessAdministrator(
            stateService.forms.mspAccessAdministrators.remove.value
        );
    ...
    if (stateService.forms.mspUsers && stateService.forms.mspUsers.add) {
        usersAdd = appUser.getAddJSONofMspUser(
            stateService.forms.mspUsers.add.value
        );
    ...
    if (stateService.forms.mspUsers && stateService.forms.mspUsers.update) {
        usersEdit = appUser.getEditJSONofMspUser(
            stateService.forms.mspUsers.update.value
        );
    ...
    if (stateService.forms.mspUsers && stateService.forms.mspUsers.remove) {
        usersRemove = appUser.getRemoveJSONofMspUser(
            stateService.forms.mspUsers.remove.value
        );
    ...
    if (stateService.forms.mspGroups && stateService.forms.mspGroups.add) {
        groupsAdd = appGroup.getAddJSONofMspGroup(
            stateService.forms.mspGroups.add.value
        );
    ...
    if (stateService.forms.mspGroups && stateService.forms.mspGroups.update) {
        groupsEdit = appGroup.getEditJSONofMspGroup(
            stateService.forms.mspGroups.update.value
        );
    ...
    if (stateService.forms.mspGroups && stateService.forms.mspGroups.remove) {
        groupsRemove = appGroup.getRemoveJSONofMspGroup(
            stateService.forms.mspGroups.remove.value
        );
    ...
    // // console.log(organization);
    if (isValidOptionalField(organization)) {
        jsonPayLoad.org_maintenance = organization;
    }

    if (isValidOptionalField(singingAuthorityAdd)) {
        jsonPayLoad.signing_authority_added = singingAuthorityAdd;
    }
    if (isValidOptionalField(singingAuthorityEdit)) {
        jsonPayLoad.signing_authority_updated = singingAuthorityEdit;
    }
    if (isValidOptionalField(singingAuthorityRemove)) {
        jsonPayLoad.signing_authority_removed = singingAuthorityRemove;
    }

    if (isValidOptionalField(accessAdministratorAdd)) {
        jsonPayLoad.access_administrator_added = accessAdministratorAdd;
    }
    if (isValidOptionalField(accessAdministratorEdit)) {
        jsonPayLoad.access_administrator_updated = accessAdministratorEdit;
    }
    if (isValidOptionalField(accessAdministratorRemove)) {
        jsonPayLoad.access_administrator_removed = accessAdministratorRemove;
    }

    if (isValidOptionalField(usersAdd)) {
        jsonPayLoad.users_added = usersAdd;
    }
    if (isValidOptionalField(usersEdit)) {
        jsonPayLoad.users_updated = usersEdit;
    }
    if (isValidOptionalField(usersRemove)) {
        jsonPayLoad.users_removed = usersRemove;
    }

    if (isValidOptionalField(groupsAdd)) {
        jsonPayLoad.msp_group_added = groupsAdd;
    }
    if (isValidOptionalField(groupsEdit)) {
        jsonPayLoad.msp_group_updated = groupsEdit;
    }
    if (isValidOptionalField(groupsRemove)) {
        jsonPayLoad.msp_group_removed = groupsRemove;
    }
