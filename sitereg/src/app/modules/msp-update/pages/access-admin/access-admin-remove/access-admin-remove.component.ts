...
import { cUpdateAccessAdminValidator } from '../shared/access-admin-shared';
...
export class MspDirectUpdateAccessAdministratorRemoveComponent
    ...
    private createArrayForm() {
        const form = this.fb.group({
            emailAddress: [
                null,
                cUpdateAccessAdminValidator.remove.emailAddress,
            ],
            ministryUserId: [
                null,
                cUpdateAccessAdminValidator.remove.ministryUserId,
            ],
        });
    ...
    patchValue(formGroup) {
        ...
        formGroup.patchValue(
            RandomObjects.getRemoveUser(
                (this.getFormsCount + 1).toString() + 'AA'
            )
        );
    }
}
